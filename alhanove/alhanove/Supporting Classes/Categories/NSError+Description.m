...
+ (NSError *)errorForLoginResponse:(NSDictionary *)response andError:(NSError *)error
{
    id message = response[@"error_message"];
    if (message) {
        if ([message isKindOfClass:[NSString class]]) {
            return [NSError errorWithDescription:message];
        } else {
