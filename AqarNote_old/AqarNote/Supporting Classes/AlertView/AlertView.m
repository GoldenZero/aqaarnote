...
#import "AlertButtonContainerView.h"
...
static AlertBackgroundWindow *__cx_alert_background_window;
static AlertView *__cx_alert_current_view;
...
@property (nonatomic, strong) AlertButtonContainerView *bottomScrollView;
...
// Buttons
- (UIFont*)fontForButtonType:(AlertViewButtonType)type;
- (void)addButtonWithTitle:(NSString *)title type:(AlertViewButtonType)type handler:(AlertButtonHandler)handler font:(UIFont *)font;
- (AlertButtonItem *)buttonItemWithType:(AlertViewButtonType)type font:(UIFont *)font;
- (void)buttonAction:(AlertButtonItem *)buttonItem;
- (void)setButtonImage:(UIImage *)image forState:(UIControlState)state andButtonType:(AlertViewButtonType)type;
...
+ (void)initialize
{
    ...
    dispatch_once(&onceToken, ^ {
        AlertView *appearance = [self appearance];
...
- (id)initWithTitle:(NSString *)title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle WithFont:(NSString*)fontName
{
    ...
    messageLabel.font = [UIFont fontWithName:fontName size:14.0];
...
- (id)initWithTitle:(NSString *)title contentView:(UIView *)contentView cancelButtonTitle:(NSString *)cancelButtonTitle
{
    ...
    if (self) {
        ...
        if (cancelButtonTitle) {
            [self addButtonWithTitle:cancelButtonTitle type:AlertViewButtonTypeCancel handler:^(AlertView *alertView, AlertButtonItem *button) {
                [alertView dismiss];
            }];
        }
    }
...
{
    [self setButtonImage:defaultButtonImage forState:state andButtonType:AlertViewButtonTypeDefault];
}

- (void)setCancelButtonImage:(UIImage *)cancelButtonImage forState:(UIControlState)state
{
    [self setButtonImage:cancelButtonImage forState:state andButtonType:AlertViewButtonTypeCancel];
}

- (void)setCustomButtonImage:(UIImage *)customButtonImage forState:(UIControlState)state
{
    [self setButtonImage:customButtonImage forState:state andButtonType:AlertViewButtonTypeCustom];
}
// AlertView action
- (void)show
{
...
{
    [[AlertView sharedQueue] removeAllObjects];
}
...
+ (void)showBackground
{
    if (!__cx_alert_background_window) {
        __cx_alert_background_window = [[AlertBackgroundWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)setupScrollViews
{
    ...
    if (!_bottomScrollView) {
        _bottomScrollView = [[AlertButtonContainerView alloc] init];
...
{
	UIFont *font = nil;
	if(type==AlertViewButtonTypeCancel)
	{
		font = self.cancelButtonFont;
	}
	else if(type==AlertViewButtonTypeCustom)
	{
...
-(CGFloat)heightThatFitsButton:(AlertButtonItem*)button
{
...
-(void)setMaxSizeForAllButtons
{
	...
	for(AlertButtonItem *button in self.buttons)
	{
...
- (void)addButtonWithTitle:(NSString *)title type:(AlertViewButtonType)type handler:(AlertButtonHandler)handler font:(UIFont *)font
{
    AlertButtonItem *button = [self buttonItemWithType:type font:font];
    button.title = title;
	...
	else
	{
		...
		AlertButtonItem *firstButton = [_buttons objectAtIndex:0];
...
- (AlertButtonItem *)buttonItemWithType:(AlertViewButtonType)type font:(UIFont *)font
{
	AlertButtonItem *button = [AlertButtonItem buttonWithType:UIButtonTypeCustom];
//	button.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    button.titleLabel.font = font;
	UIImage *normalImage = nil;
	UIImage *highlightedImage = nil;
	switch (type) {
		case AlertViewButtonTypeCancel:
			...
			break;
		case AlertViewButtonTypeCustom:
			...
			break;
		case AlertViewButtonTypeDefault:
		default:
...
- (void)setButtonImage:(UIImage *)image forState:(UIControlState)state andButtonType:(AlertViewButtonType)type
{
    for (AlertButtonItem *button in _buttons)
    {
        if(button.type == type)
        {
...
- (void)setButtonColor:(UIColor *)buttonColor
{
    ...
    [self setColor:buttonColor toButtonsOfType:AlertViewButtonTypeDefault];
}
...
- (void)setCancelButtonColor:(UIColor *)buttonColor
{
    ...
    [self setColor:buttonColor toButtonsOfType:AlertViewButtonTypeCancel];
}
...
- (void)setCustomButtonColor:(UIColor *)buttonColor
{
    ...
    [self setColor:buttonColor toButtonsOfType:AlertViewButtonTypeCustom];
}
...
-(void)setColor:(UIColor *)color toButtonsOfType:(AlertViewButtonType)type {
    for (AlertButtonItem *button in _buttons) {
        if (button.type == type) {
